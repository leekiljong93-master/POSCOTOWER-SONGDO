...
if len(st.session_state.projects) > 1:
    ...
    st.sidebar.button("🗑️ 현재 프로젝트 삭제", disabled=True, width="stretch")
...
with tab1:
    ...
    with col1:
        st.markdown("##### 🧱 자재비")
        df_mat = db.get_filtered_master_items(sheet_name="자재비")
        mat_names = df_mat['item_name'].tolist() if not df_mat.empty else ["데이터 없음"]
        m_sel = st.selectbox("품목 선택", mat_names, key="mat_select")
        m_qty = st.number_input("수량", min_value=0.0, step=1.0, format="%.2f", key="mat_qty")
        if st.button("자재비 추가", key="mat_add"):
            u_price = db.get_unit_price(m_sel, sheet_name="자재비")
            new_row = pd.DataFrame(
                {"공종명": [m_sel], "구분": ["자재"], "단위": ["식"], "단가": [u_price], "수량": [m_qty], "합계": [u_price * m_qty],
    ...
    with col2:
        st.markdown("##### 👷 인건비")
        df_lab = db.get_filtered_master_items(sheet_name="인건비")
        lab_names = df_lab['item_name'].tolist() if not df_lab.empty else ["데이터 없음"]
        l_sel = st.selectbox("직종 선택", lab_names, key="lab_select")
        l_qty = st.number_input("인원", min_value=0.0, step=1.0, format="%.2f", key="lab_qty")
        if st.button("인건비 추가", key="lab_add"):
            u_price = db.get_unit_price(l_sel, sheet_name="인건비")
            new_row = pd.DataFrame(
                {"공종명": [l_sel], "구분": ["노무"], "단위": ["인"], "단가": [u_price], "수량": [l_qty], "합계": [u_price * l_qty],
                 "시작일": [date.today()], "종료일": [date.today()]})
            st.session_state.estimate_data = pd.concat([st.session_state.estimate_data, new_row], ignore_index=True)
            st.rerun()
    ...
    with col3:
        st.markdown("##### 🏗️ 장비비")
        df_eq = db.get_filtered_master_items(sheet_name="장비비")
        eq_names = df_eq['item_name'].tolist() if not df_eq.empty else ["데이터 없음"]
        e_sel = st.selectbox("장비 선택", eq_names, key="eq_select")
        e_qty = st.number_input("시간", min_value=0.0, step=1.0, format="%.2f", key="eq_qty")
        if st.button("장비비 추가", key="eq_add"):
            u_price = db.get_unit_price(e_sel, sheet_name="장비비")
            new_row = pd.DataFrame(
                {"공종명": [e_sel], "구분": ["장비"], "단위": ["시간"], "단가": [u_price], "수량": [e_qty], "합계": [u_price * e_qty],
                 "시작일": [date.today()], "종료일": [date.today()]})
            st.session_state.estimate_data = pd.concat([st.session_state.estimate_data, new_row], ignore_index=True)
            st.rerun()
    ...
    with col4:
        st.markdown("##### 📦 세트")
        df_set = db.get_filtered_master_items(sheet_name="세트")
        ...
        if st.button("세트 추가", key="set_add"):
            u_price = db.get_unit_price(s_sel, sheet_name="세트")
            new_row = pd.DataFrame(
                {"공종명": [s_sel], "구분": ["자재"], "단위": ["식"], "단가": [u_price], "수량": [1.0], "합계": [u_price * 1.0],
                 "시작일": [date.today()], "종료일": [date.today()]})
            st.session_state.estimate_data = pd.concat([st.session_state.estimate_data, new_row], ignore_index=True)
            st.rerun()
    ...
    if not st.session_state.estimate_data.empty:
        ...
        st.download_button(label="📊 견적서 엑셀 다운로드", data=output.getvalue(),
...
with tab3:
    ...
    if st.button("💾 전체 변경사항 구글 시트에 자동 분리 저장", type="primary", use_container_width=True):
        if search_kw != "":
            st.warning("⚠️ 검색어가 입력된 상태에서는 부분 저장되어 데이터가 유실될 수 있습니다. 검색어를 비우고 전체 목록 상태에서 저장해주세요.")
        else:
            with st.spinner("데이터를 각 시트(자재/인건/장비/세트)별로 똑똑하게 분리하여 저장하는 중..."):
                try:
                    final_df = edited_master.copy()
                    # 이름이나 구분이 없는 빈 줄은 걸러냄
                    final_df = final_df.dropna(subset=["item_name", "구분"])
